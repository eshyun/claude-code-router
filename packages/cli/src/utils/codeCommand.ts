...
export async function executeCodeCommand(
  ...
  // Build args array properly - shell-quote handles escaping
  const claudeArgs = args.length > 0 ? quote(args).split(' ').filter(Boolean) : [];
  ...
  const claudeProcess = spawn(
    ...
    claudeArgs,
    {
      ...
      stdio: stdioConfig,
      // shell: false (default) - pass args as array for security
    }
  );
